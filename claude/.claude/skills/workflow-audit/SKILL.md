---
name: workflow-audit
description: Use when the user asks to audit recent Claude work, review prior projects or memories, identify repeated manual workflows, or propose new skills, subagents, hooks, commands, scripts, or automations from recurring patterns.
---

# Workflow Audit

Find recurring manual work in recent Claude activity and turn it into concrete reusable artifacts. Treat this as evidence gathering, not brainstorming from vibes.

## When to use

Use this when the user asks to:
- audit recent work for skill or automation opportunities
- inspect prior projects, memories, transcript files, or recurring manual steps
- decide whether a workflow should become a skill, subagent, hook, command, script, or automation

Do not use this for normal code review, one-off tech debt audits, or generic productivity advice.

## Procedure

1. Define the audit window. If the user did not specify one, inspect the most recent relevant project memory and current task context before expanding further.
2. Gather evidence from the narrowest useful sources:
   - relevant files under `claude/.claude/projects/*/memory/`
   - directly relevant project transcript files under `claude/.claude/projects/`
   - existing artifacts under `claude/.claude/skills/`, `claude/.claude/hooks/`, and command directories when present
...
4. Classify each candidate:
   ...
   - **Hook or command**: enforceable or invocable workflow that should run consistently
...
- Do not read broad history dumps when targeted searches and relevant project memories answer the question.
- Do not recommend a skill when a script, hook, command, or automation would make the behavior deterministic.
