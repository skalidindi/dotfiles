...
/**
 * Prefer a small extraction model when available, otherwise use the current
 * session model so the command survives model catalog changes.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api>> {
	const codexModel = modelRegistry.find("nflx-openai", CODEX_MODEL_ID);
	if (codexModel) {
	...
	const haikuModel = modelRegistry.find("nflx-anthropic", HAIKU_MODEL_ID);
	if (haikuModel) {
