...
function commandLooksObsolete(command: string): boolean {
  return (
    /https?:\/\/[a-zA-Z0-9-]+\.vip\.[a-zA-Z0-9-]+\.test\.cloud\.netflix\.net/.test(command) ||
    /(?:^|\n|[;|&]{1,2})\s*grpcurl\b/m.test(command)
  );
}
...
export function registerZoneExecApprovalDiscipline(pi: ExtensionAPI, reminders: ReminderEmitter): void {
  ...
  pi.on("tool_result", async (event) => {
    ...
    if (toolNameMatches(toolName, "zone_exec") && !event.isError) {
      ...
      const cancellationAllowed = commandLooksObsolete(command) || queuedResultSuggestsCancellation(text);
